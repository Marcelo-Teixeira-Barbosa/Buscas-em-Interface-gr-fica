...
public class PathFinding {
	...
	//UTIL ARRAYS
	private String[] algorithms = {"Dijkstra","A*", "Largura","Profundidade","Cust Uniforme","Melhor Escolha"};
...
//asdsdaasd
		...
		public void BME() {
			...
			while(solving) {
				...
				sortHeur(priority);	//SORT THE PRIORITY QUE
			}
		}
		
		public ArrayList<Node> sortQue(ArrayList<Node> sort) {	//SORT PRIORITY QUE (Distance + Heuristic)
			int c = 0;
			while(c < sort.size()) {
				int sm = c;
		...
		public ArrayList<Node> sortHeur(ArrayList<Node> sort) {	//SORT PRIORITY QUE (Heuristic + Distance)
			int c = 0;
			while(c < sort.size()) {
				int sm = c;
				for(int i = c+1; i < sort.size(); i++) {
		...
		public ArrayList<Node> sortCost(ArrayList<Node> sort) {	//SORT PRIORITY QUE (Distance Only)
			int c = 0;
			while(c < sort.size()) {
				int sm = c;
				for(int i = c+1; i < sort.size(); i++) {
					if(sort.get(i).getEuclidDist() < sort.get(sm).getEuclidDist())
						sm = i;
				}
				if(c != sm) {
					Node temp = sort.get(c);
